...
class PET_Server:
    ...
    def prepare_inputs(self, batch_size, seq_len):
        self.batch_size = batch_size
        self.seq_len = seq_len
        #task_ids = torch.LongTensor([0] + [1] * (self.batch_size-1))
        task_ids = torch.LongTensor([0,1] * (self.batch_size//2))
...
if __name__ == '__main__':
    ...
    for batch_size in [2, 4, 8, 16, 32, 64, 128]:
        for seq_len in [4,8,16,32,64, 128,256]:
            ...
            print(f"batch_size:{batch_size},seq_len:{seq_len},numstreams:1,time:{median_time_1stream:.4f}")
            print(f"batch_size:{batch_size},seq_len:{seq_len},numstreams:2,time:{median_time_2stream:.4f}")
            print(f"{'='*50}")
            # Log results to file
            with open('cuda_benchmark_results.log', 'a') as f:
                f.write(f"batch_size:{batch_size},seq_len:{seq_len},numstreams:1,time:{median_time_1stream:.4f}\n")
                f.write(f"batch_size:{batch_size},seq_len:{seq_len},numstreams:2,time:{median_time_2stream:.4f}\n")
