...
class PET_Server:
    ...
    def prepare_inputs(self,k):
        self.batch_size = 4*k
        self.seq_len = 32
        task_ids = torch.LongTensor([0,1,2,3]*k)
        n_samples = torch.LongTensor([1,1,1,1]*k)
        input_ids = torch.randint(low=0,
                                  high=self.cfg.vocab_size - 1,
...
if __name__ == '__main__':
    ...
    for seed in range(410,415):
        ...
        for k in [1,2,4,8]:
            ...
            inputs = server.prepare_inputs(k)
            mem_after_load = turbo_transformers.get_gpu_mem_usage()
            for i in [1,2]:
                total_time = server.run(inputs,i)
                with open("muiti_streams_time.txt", "a") as f:
                    f.write(f"streams_num:{i},num_k:{k},total_time:{ str(total_time)}\n")  # 转为字符串写入文件
