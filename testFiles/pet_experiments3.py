...
class PET_Server:
    ...
    def run(self, no_log = False, bs = 32, current_query_pool = None):
        turbo_transformers.set_num_streams(self.cfg.num_streams)
        cur_query_id = 0
        while(cur_query_id < len(current_query_pool)):
            # 取出当前时间内的query
            cur_time = self.current_time
            cur_query_time = self.pending_queue[self.query_ptr][4]
            self.current_time = max(cur_query_time, cur_time)
            self.query_ptr += 1
            cur_query_id += 1
            ...
            for mean_v in [32]:
                for std_v in [1,2,4,8]:
                    ...
                    for i in range(len(self.arrival_time_list)):
                        current_query_pool.append(self.query_pool[i])
                        if((i + 1) % 512 == 0):
