...
class PET_Server:
    ...
    def prepare_tasks(self):
        ...
        random.seed(self.cfg.seed)
        for _ in tqdm.tqdm(range(num_tasks//2)):
            pet_type = 3
    ...
    def compare_multi_stream(self):
        ...
        self.init()
        self.cfg.total_queries = 128
        for task_num in [8]:
            self.cfg.num_tasks = task_num
            for i in range(1):
                for seq_len in [32,64,96,128,160,192,224,256]:
                    self.cfg.mean_v = seq_len
                    self.cfg.std_v = 0
                    self.prepare_query_pool()
                    for stream_num in [8, 4, 2, 1]:
                        ...
                        self.run(bs = self.cfg.total_queries)
